...
import {SystemInfoService} from "../../../share-datas/systemInfo.service";
...
export class SystemWordChoiceComponent implements OnInit{
  @Output()
  chooseSystemInfo = new EventEmitter<SystemInfo | undefined>();
  systemInfoList: SystemInfo[] | undefined;
  constructor(private systemInfoService: SystemInfoService) {
    this.systemInfoList = this.systemInfoService.systemInfoList;
  }
  @Output()
  ...
  fill(){
    if(this.systemInfoList?.length===0) {
  ...
  release(info?: SystemInfo) {
    this.chooseSystemInfo.emit(info);
